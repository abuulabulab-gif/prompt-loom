...
export const TAG_DICT = {
  ...
  'colored inner hair':    { ja: 'インナーカラーの実在タグ版。襟足や耳裏など内側の髪だけ別色にする', en: 'Danbooru-standard tag for inner-layer hair in a different color' },
  'two-tone hair':         { ja: '髪が2色で構成されるスタイル全般。ベース色と差し色の組み合わせで使う', en: 'Hair made of exactly two colors — pair with a base and accent color tag' },
  'multicolored hair':     { ja: '3色以上を含む多色髪の親タグ。個別の色タグと重ねて票を補強する', en: 'Parent tag for hair with multiple colors; stack with specific color tags' },
  'gradient hair':         { ja: '根元から毛先へ色がなだらかに変わるグラデーション髪', en: 'Hair color transitioning smoothly from roots to tips' },
  'split-color hair':      { ja: '左右でぱっきり色が分かれる髪。センター分けの境目で色が変わる', en: 'Hair split into two colors down the middle — left/right halves differ' },
  'streaked hair':         { ja: '一房だけ別色のメッシュ・ハイライト染め', en: 'Hair with contrasting streaks or highlights in select strands' },
  ...
  'highleg':               { ja: 'ハイレグ。脚の付け根が腰骨近くまで切れ上がったデザイン。レオタード・水着と好相性で、脚を長く見せる', en: 'High-cut leg opening rising to the hips — elongates the legs; classic on leotards and swimsuits' },
  'halterneck':            { ja: 'ホルターネック。首の後ろで紐や布を結んで吊るデザイン。肩と背中が大きく開く', en: 'Garment tied behind the neck, leaving shoulders and back open' },
  'criss-cross halter':    { ja: 'クロスホルター。首元〜胸元で紐がX字に交差するホルター。デコルテの情報量が増える', en: 'Halter with straps crossing in an X over the chest or neck' },
  'plunging neckline':     { ja: '深Vネック。谷間〜みぞおちまで深く切れ込んだ大胆な襟ぐり', en: 'Neckline plunging deep toward the sternum' },
  'single bare shoulder':  { ja: '片肩出し。片方の肩だけ露出した非対称の着こなし。ワンショルダー服や着崩しで出る', en: 'One shoulder exposed — asymmetric one-shoulder garments or slipped clothing' },
  'asymmetrical clothes':  { ja: '左右非対称の衣装。丈・袖・デザインが左右で違う。非対称コーデの土台になるタグ', en: 'Left-right asymmetric outfit design — the base tag for asymmetric styling' },
  'heart cutout':          { ja: 'ハート型のくり抜き窓。胸元に開けるのが定番', en: 'Heart-shaped cutout, typically at the chest' },
  'zipper':                { ja: 'ジッパー。前開きファスナーの見た目。unzipping（着脱・動作）と組むと半開きの表現に', en: 'Visible zipper detail — pair with unzipping for a half-open look' },
  'o-ring':                { ja: 'Oリング。金属の輪で布や紐を留めるディテール。チョーカーや水着の胸元に定番', en: 'Metal O-ring hardware joining straps or fabric — common on chokers and swimwear' },
  'cross-laced clothes':   { ja: '編み上げ。コルセットのように紐をX字に通して締めるデザイン', en: 'Corset-style criss-cross lacing on clothing' },
  'translucent':           { ja: '半透明。単体では控えめだが、see-through（シースルー）と重ねると透け表現の同義補強になる', en: 'Semi-transparent material — stack with see-through to reinforce transparency' },
  ...
  'frilled bikini':        { ja: 'フリル（ひらひら）付きのビキニ。可愛い・フェミニン・アニメ系の水着シーンで定番。通常のビキニより華やかな印象に', en: 'Bikini with frilled edges — a staple of cute and feminine anime beach scenes; more decorative than a plain bikini' },
  'string bikini':         { ja: '極細のひも状ストラップのビキニ。マイクロビキニよりは布量があるが肌の露出が多く大人っぽい印象に', en: 'Bikini with very thin string straps — more coverage than a micro bikini but still highly revealing; mature and sensual look' },
  'monokini':              { ja: 'モノキニ。ワンピース水着の側面・前面を大きくくり抜いたデザイン、またはビキニボトムに最小限のトップを繋げた超露出型水着', en: 'One-piece swimsuit with large cutouts, or a bikini bottom with a minimal connected top — high-exposure statement swimwear' },
  'rashguard':             { ja: 'ラッシュガード。サーフィン・水泳用のUVカット長袖（または半袖）トップ。水着の上から着て日焼けを防ぐ。スポーティな夏キャラに', en: 'UV-protective surf top worn over swimwear — long or short sleeve; sporty and active summer character look' },
  'pareo':                 { ja: 'パレオ（サロン）。ビーチで水着の上に巻く薄手の布。腰に巻いてスカート風にしたり肩に羽織ったりと自由にアレンジできる', en: 'Sheer wrap worn over a swimsuit at the beach — around the waist as a skirt or draped over the shoulders' },
  ...
  'capelet':               { ja: '肩だけを覆う短いマント。クロークより軽く、制服や軍装の上に重ねる装飾向き', en: 'Short cape covering only the shoulders — lighter than a cloak, layers over uniforms' },
  'high collar':           { ja: '立襟・詰襟。首に沿って立ち上がる襟。軍服・制服・和洋折衷の衣装の芯になる', en: 'Standing collar rising along the neck — military, uniform, or formal styling' },
  ...
  'single sock':           { ja: '片脚だけソックスを履いた状態。もう片方は素足。抜け感・ちぐはぐな魅力を作る', en: 'A sock on one leg only, the other bare — relaxed, quirky asymmetry' },
  'single leg pantyhose':  { ja: '片脚だけストッキングを着けた非対称スタイル。デザイン性の高い衣装に合う', en: 'Pantyhose on one leg only — a bold asymmetric styling choice' },
  'asymmetrical legwear':  { ja: '左右非対称のレッグウェア全般。丈・柄・種類の違いをまとめて指示する親タグ', en: 'General tag for asymmetric legwear — covers length, pattern, and type differences' },
  'over-kneehighs':        { ja: '膝上〜太もも中間の丈の靴下。ニーソックスとニーハイの中間で絶対領域が細く出る', en: 'Socks reaching just above the knee — between knee-highs and thigh-highs' },
  'striped thighhighs':    { ja: '縞模様のニーハイ。定番の魔女っ子・小悪魔系アイテムで脚のラインを強調する', en: 'Striped thigh-highs — a classic witchy or impish item that accentuates the legs' },
  ...
  'single detached sleeve': { ja: '片腕だけの分離袖。肩が出たまま袖だけが独立して付く非対称デザイン', en: 'A detached sleeve on one arm only — shoulder stays bare' },
  'asymmetrical sleeves':  { ja: '左右で形の違う袖。片方長袖＋片方ノースリーブのような大胆な非対称を作る', en: 'Sleeves of different designs on each arm — e.g., one long, one sleeveless' },
  'uneven sleeves':        { ja: '左右で袖の丈が違うデザイン。さりげない非対称で個性を出す', en: 'Sleeves of different lengths on each arm — subtle asymmetry' },
  'mismatched sleeves':    { ja: '左右で色や柄が違う袖。ちぐはぐさを狙ったデザイン性の高い一着になる', en: 'Sleeves with different colors or patterns on each arm' },
  'single glove':          { ja: '片手だけの手袋。主人公感・こだわりの演出として定番の非対称アイテム', en: 'A glove on one hand only — an iconic asymmetric accent' },
  'single elbow glove':    { ja: '片腕だけの肘上ロンググローブ。ドレスやステージ衣装の非対称演出に効く', en: 'An elbow-length glove on one arm only — elegant asymmetry' },
  'mismatched gloves':     { ja: '左右で違うデザインの手袋。色違い・柄違いでキャラの個性を作る', en: 'Gloves of different designs on each hand' },
  'uneven gloves':         { ja: '左右で丈の違う手袋。片方ロング＋片方ショートの非対称スタイル', en: 'Gloves of different lengths on each hand' },
  'single shoe':           { ja: '片足だけ靴を履いた状態。もう片方は素足や靴下のまま。演出的な抜け感が出る', en: 'A shoe on one foot only — theatrical, undone asymmetry' },
  'single boot':           { ja: '片足だけブーツを履いた非対称スタイル。衣装デザインの強いアクセントになる', en: 'A boot on one foot only — a strong asymmetric accent' },
  'mismatched footwear':   { ja: '左右で違う靴を履いた状態。色違い・種類違いでちぐはぐな個性を演出する', en: 'Different shoes on each foot — quirky mismatched styling' },
  'high-low skirt':        { ja: '前が短く後ろが長いスカート。フィッシュテールとも呼ばれ、脚見せと優雅さを両立する', en: 'Skirt short in front, long in back (fishtail) — legs plus elegance' },
  'asymmetrical dress':    { ja: '左右非対称のドレス。裾の斜めカットや片肩デザインで動きのあるシルエットを作る', en: 'Asymmetric dress — diagonal hems or one-shoulder cuts create dynamic silhouettes' },
  'single earring':        { ja: '片耳だけのイヤリング。さりげない非対称でこなれ感・キャラの目印を作る', en: 'An earring on one ear only — subtle signature asymmetry' },
  'asymmetrical bangs':    { ja: '左右非対称の前髪。長さや流しの向きが左右で違い、顔に個性が出る', en: 'Bangs of uneven length or sweep — adds facial character' },
  'asymmetrical hair':     { ja: '左右非対称のヘアスタイル全般。片側だけ長い・刈り上げなど大胆な髪型に', en: 'Asymmetric hairstyle overall — one side long, undercut, etc.' },
  'single hair bun':       { ja: '片側だけのお団子ヘア。左右非対称のかわいさを作る定番アレンジ', en: 'A hair bun on one side only — classic asymmetric-cute arrangement' },
