...
const GUIDE = (lang) => [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: lang === 'ja' ? '🩺 健康診断' : '🩺 Health', text: lang === 'ja' ? 'キャラパネルの各カード右側に「良／小／中／大」のバッジでプロンプトの健康診断を表示。【判定基準】①競合ルール：矛盾タグはエラー、珍しい組み合わせはソフト警告。②ブロック過密：1ブロック14タグ以上でソフト警告。③一貫性チェック（同じキャラが毎回生成されるか）：髪色未設定・目の色未設定・タグ数8未満のいずれかがあるとソフト警告として加算。【評価】エラー2件以上→大、エラー1件またはソフト警告2件以上→中、ソフト警告1件→小、問題なし→良。ツールチップで「競合 N件 / 髪色未設定」など詳細も確認できる' : 'Each character card in the panel shows a health badge (良/小/中/大) for the current prompt. Scoring: ① Conflict rules — hard conflicts = errors; unusual-but-possible = soft warnings. ② Block crowding — 14+ tags in one block = soft warning. ③ Consistency check (will the same character appear each time?): missing hair color, missing eye color, or fewer than 8 total tags each count as a soft warning. Result: 2+ errors→大, 1 error or 2+ soft→中, 1 soft→小, clean→良. Tooltip shows details like "競合 1件 / 髪色未設定".' },
          ...
          {tab === 'about' && (
            <div className="px-[1.125rem] py-[1.125rem] space-y-5">
              ...
              <div>
                ...
                <div className="space-y-[0.3125rem]">
                  {[
                    { v: 'v2.8', note: lang === 'ja' ? '✂️カットメーカー拡張：新グループ「留め・編み上げ」（ジッパー・Oリング・編み上げ）を追加。シルエット変形にハイレグ・ホルターネック・クロスホルター・深Vネック・片肩出し・アシンメトリー衣装、開口部・窓にハートくり抜き、素材・質感に半透明（シースルーと重ねる同義補強向け）を追加——全11タグにタグ辞書の解説つき。採用タグはDanbooru実在＆投票数を確認済み（単体で票が入らない語は見送り）。⭐基本品質ブロックが最初から折りたたまれた状態に：定番の品質タグ（masterpiece等）は最初から選択済みで出力に入るので、開いて調整するのはこだわりたい人だけでOK——新規キャラ作成時に最初に目に入るのが「キャラ作り」になるように。🌓左右メーカー新設：片方だけ（片手袋・片ソックス・片耳イヤリング等）・左右で丈違い・左右別デザインを部位ごと（髪/肩袖/手袋/レッグ/靴/服全体/アクセ）に選んで、それぞれの持ち場のブロックへ一括追加。衣装・衣装ディテールブロックと✦ツールメニューに🌓ボタン。静的タグ拡充：袖4種（片分離袖・左右違い袖ほか）・手袋4種・レッグ5種（片脚ストッキング・オーバーニー・縞ニーハイ等）・靴3種（片靴・左右違い靴等）・前短後長スカート・アシンメトリードレス・片耳イヤリング・アシメ前髪/ヘア・片側お団子（全てタグ辞書に解説つき）' : '✂️ Cutout Maker expanded: new "Fastening" group (zipper, o-ring, cross-laced), plus highleg, halterneck, criss-cross halter, plunging neckline, single bare shoulder, and asymmetrical clothes in Silhouette, heart cutout in Cutouts, and translucent in Texture (stack with see-through for reinforcement) — all 11 tags with dictionary entries, each verified against Danbooru post counts (dead-vote words skipped). ⭐ Quality block now starts collapsed: proven defaults (masterpiece etc.) are pre-selected and always included in output, so newcomers land straight on character building — open it only when you want to fine-tune. 🌓 Asymmetry Maker: pick single-side (single glove, single sock, single earring…), uneven-length, and mismatched designs per body zone (hair / sleeves / gloves / legwear / shoes / whole outfit / accessories) — tags route to their home blocks in one apply. 🌓 button on Outfit & Outfit Detail blocks and in the ✦ Tools menu. New static tags with dictionary entries: 4 sleeve, 4 glove, 5 legwear (single leg pantyhose, over-kneehighs, striped thighhighs…), 3 footwear, high-low skirt, asymmetrical dress, single earring, asymmetrical bangs/hair, single hair bun.' },
