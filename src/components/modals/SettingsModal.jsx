...
const MOBILE_TIPS = (lang) => [
  ...
  },
  {
    icon: '🌓',
    text: lang === 'ja'
      ? '✦ツール → 🌓左右メーカーで「片方だけ・左右で丈違い・左右別デザイン」を部位ごと（髪・肩袖・手袋・レッグ・靴・服全体・アクセ）に選択。タグはそれぞれの持ち場のブロックへ一括で入る。衣装・衣装ディテールブロックの🌓ボタンからも起動できる'
      : '✦ Tools → 🌓 Asymmetry Maker picks single-side, uneven-length, and mismatched designs per body zone (hair, sleeves, gloves, legwear, shoes, whole outfit, accessories) — tags route to their home blocks in one apply. Also launch from the 🌓 button on Outfit / Outfit Detail blocks',
  },
  {
    icon: '💡',
    text: lang === 'ja'
...
const GUIDE = (lang) => [
  {
    ...
    items: [
      ...
      { icon: lang === 'ja' ? '✦ ツール' : '✦ Tool', text: lang === 'ja' ? 'ヘッダーの「✦ ツール」からテンプレート・🎨カラーメーカー・🎯特徴メーカー・🧵マテリアルメーカー・✂️カットメーカー・🌓左右メーカー・キャラ共演・AIタグ生成へアクセス' : '"✦ Tools" in the header opens Template, 🎨 Color Maker, 🎯 Feature Maker, 🧵 Material Maker, ✂️ Cutout Maker, 🌓 Asymmetry Maker, Collab, and AI tag generation' },
  ...
  },
  {
    ...
    items: [
      ...
      { icon: lang === 'ja' ? '🩺 健康診断' : '🩺 Health', text: lang === 'ja' ? 'キャラパネルの各カード右側に「良／小／中／大」のバッジでプロンプトの健康診断を表示。【判定基準】①競合ルール：矛盾タグはエラー、珍しい組み合わせはソフト警告。②ブロック過密：1ブロック14タグ以上、または全体46タグ以上でソフト警告（多すぎると1タグあたりの効きが薄まる）。③一貫性チェック（同じキャラが毎回生成されるか）：髪色未設定・目の色未設定・タグ数8未満・（女性キャラの）胸サイズ未設定・素体の抜け（肌色・身長感＝まとめて1件）が各1件のソフト警告。④同じタグが複数ブロックに重複していたら1件（票の無駄・強度が割れる）。【評価】エラー2件以上→大、エラー1件またはソフト警告2件以上→中、ソフト警告1件→小、問題なし→良。ツールチップで「競合 N件 / 素体の抜け: 肌色」など詳細も確認できる' : 'Each character card in the panel shows a health badge (良/小/中/大) for the current prompt. Scoring: ① Conflict rules — hard conflicts = errors; unusual-but-possible = soft warnings. ② Crowding — 14+ tags in one block, or 46+ tags overall (votes get diluted), each = soft warning. ③ Consistency check (will the same character appear each time?): missing hair color, missing eye color, fewer than 8 total tags, missing bust size (female chars), and base-body gaps (skin tone / height — grouped as one) each count as a soft warning. ④ The same tag duplicated across blocks = one soft warning (wasted votes, split weights). Result: 2+ errors→大, 1 error or 2+ soft→中, 1 soft→小, clean→良. Tooltip shows details.' },
