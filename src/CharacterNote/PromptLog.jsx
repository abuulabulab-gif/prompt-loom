...
export default function PromptLog({ char, lang, activeTool, posText, negText, onUpdate, onRestoreBlocks }) {
  ...
  return (
    <div>
      {/* Header */}
      <div className="flex items-center gap-2 mb-[0.6875rem] flex-wrap">
        <span className="text-fg text-xs font-bold">🗂 {lang === 'ja' ? 'プロンプトログ' : 'Prompt Log'}</span>
        <span className="text-muted text-[0.625rem] font-mono font-semibold">{log.length}{lang === 'ja' ? '件' : ' entries'}</span>
        <div className="flex-1" />
        <button
          onClick={toggleAutoLog}
          title={lang === 'ja' ? 'COPYで自動記録 ON/OFF' : 'Auto-log on COPY'}
          className="border rounded-md px-2 py-1 text-[0.625rem] font-mono cursor-pointer flex-shrink-0 flex items-center gap-1"
          style={autoLog
        ...
        <button onClick={() => setRecordOpen(true)}
          ...
          className="border-none rounded-[0.4375rem] px-[0.6875rem] py-[0.3125rem] text-[0.6875rem] font-bold cursor-pointer flex-shrink-0">
          ＋ {lang === 'ja' ? '現在のプロンプトを記録' : 'Record current'}
        </button>
      </div>

      {/* Filters */}
      <div className="flex gap-[0.3125rem] flex-wrap mb-2.5">
        <select value={filterTool} onChange={e => setFilterTool(e.target.value)}
          className="bg-bg border border-line rounded-[0.3125rem] text-[0.625rem] px-[0.4375rem] py-1 font-mono text-muted outline-none cursor-pointer">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)}
          ...
          className="bg-bg border border-line rounded-[0.3125rem] text-[0.625rem] px-2 py-1 font-mono text-fg outline-none flex-1 min-w-[6.25rem]" />
        {(filterTool !== 'all' || filterLabel || search) && (
          <button onClick={() => { setFilterTool('all'); setFilterLabel(''); setSearch(''); }}
            className="bg-transparent border border-dim rounded-[0.3125rem] px-[0.4375rem] py-1 text-[0.625rem] font-mono text-muted cursor-pointer">
            ✕
          </button>
        )}
      </div>
      ...
      {log.length > 0 && filtered.length === 0 && (
        <div className="text-muted text-[0.6875rem] font-mono text-center py-5">
      ...
      {groups.map((item, i) =>
        item.type === 'date' ? (
          <div key={`d-${i}`} className="text-muted text-[0.625rem] font-mono font-semibold py-1 mb-1.5 border-b border-line">
            ── {item.label}
          </div>
        ) : (
