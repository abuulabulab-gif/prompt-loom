...
export const CHANGELOG = [
  {
    v: 'v2.11',
    ja: '🩺破綻チェックに「着衣×乳首・乳輪」の注意を追加：服の語（白シャツ・着物・ビキニ等＝色や柄が付いていても拾う）と乳首・乳輪の語が同時にあると「そこを描く指示になり、胸がはだけやすい（見せないなら外す）」と知らせる。はだけ・透け・裸など見せる語がある時は出さない（見せるつもりの時は邪魔しない）。同じく「cameltoe」は下着やピチピチの服（パンツ・レオタード・水着・ショートパンツ等）が無い時に注意＝布が張り付いた股を描く語なので、そういう衣装の時に入れる。きっかけ＝実測で、着衣の2人に乳首の語が乗ると8枚とも胸がはだけ、外すと0枚になった。おまかせ生成には影響なし（注意だけ）。🎬キャラ共演：BREAKで区切った並びを「左→右」の指定順に揃えた＋ローカルの制作工房（HODO）を起動している時だけ「HODOに送る」ボタンが出る（公開版には出ない）',
    en: '🩺 Conflict check: new "clothed × nipple/areola" warning — when clothing tags (white shirt, kimono, bikini… colored variants included) appear with nipple/areola tags, LOOM warns that tags are draw orders and the chest may get exposed (remove them if not shown). Suppressed when exposure tags (open shirt, see-through, nude…) are present. Likewise, cameltoe now warns when no underwear/tight wear (panties, leotard, swimsuit, short shorts…) is present — it describes fabric clinging to the crotch. Prompted by a measurement: nipple tags on clothed two-character shots exposed the chest in 8/8 images; removing them brought it to 0/8. Random generation unaffected (warning only). 🎬 Collab: BREAK segments now follow the chosen left→right positions, and a "Send to HODO" button appears only when the local HODO studio is running (never on the public site).',
  },
  {
    v: 'v2.10',
