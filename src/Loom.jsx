...
export default function Loom() {
  ...
  const addCustomBlock = () => {
    ...
    const base = lang === 'ja' ? 'カスタム' : 'Custom';
    const label = customCount === 0 ? base : `${base}${customCount + 1}`;
    setCharacters(prev => prev.map(c => c.id === activeCharId ? { ...c, blocks: [...c.blocks, makeCustomBlock(label)] } : c));
  };
  ...
  const generateRandomChar = () => {
    ...
    setCharacters(prev => prev.map(c => {
      ...
      const newBlocks = c.blocks.map(block => {
        ...
        return { ...block, text, enabled: true, collapsed: false, lastRandomPicks: picks };
      });
