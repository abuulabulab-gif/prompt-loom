...
export default function Loom() {
  ...
  const paletteCommands = [
    ...
    { id: 'toggle-random-mode', group: lang === 'ja' ? 'アプリ設定' : 'App', icon: randomMode === 'chardesign' ? '🧍' : '🖼️', label: `Random mode: ${randomMode} → toggle`, labelJa: `ランダムモード切替 (現在: ${randomMode === 'chardesign' ? 'キャラ特化' : 'イラスト'})`, action: () => setRandomMode(m => { const n = m === 'chardesign' ? 'illust' : 'chardesign'; localStorage.setItem('loom_randomMode', n); return n; }) },
          ...
          >
            ...
            <div className="overflow-y-auto" style={{ maxHeight: 'calc(80vh - 2rem)', scrollbarWidth: 'thin' }}>
              {characters.map(c => {
                ...
                const hasHairColor = allText ? /\b(?:red|pink|orange|yellow|blonde|brown|black|white|gray|grey|silver|purple|blue|green|teal|gold|lavender|crimson|amber|cobalt|coral|rose|ash|dark|light|pale|navy|aqua|lime|magenta|violet|ivory|platinum|chocolate|strawberry)[\w ]*\bhair\b/i.test(faceT) : true;
                const hasEyeColor  = allText ? /\b(?:red|pink|orange|yellow|brown|black|white|gray|grey|silver|purple|blue|green|teal|gold|lavender|amber|violet|dark|light|pale|aqua|navy|lime|crimson)[\w ]*\beyes\b/i.test(faceT) : true;
              ...
              <div className="flex-shrink-0 flex items-center gap-[0.3125rem] mb-[0.3125rem]">
                <div className="relative flex-shrink-0">
                  <button onClick={() => setToolPickerOpen(p => !p)} title={lang === 'ja' ? tool.note : tool.noteEn}
                    ...
                    {tool.icon} {lang === 'ja' ? tool.name : tool.nameEn}<span className="text-[0.5625rem] ml-0.5">▾</span>
                  </button>
                  {toolPickerOpen && (
                    <>
