...
const GUIDE = (lang) => [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: lang === 'ja' ? '⊞ 一覧で編集' : '⊞ Grid edit', text: lang === 'ja' ? 'プロフィールシートの⊞ボタンで一覧モード：全セクション・全項目をアコーディオン無しで1画面に並べ、Enterキーで次の欄へ移動しながら一気に書き込める（スプレッドシート感覚）。タグ欄・項目の追加削除はフォーム表示側で' : '⊞ button on the Profile Sheet opens Grid mode — every section and field on one screen, Enter jumps to the next field (spreadsheet-style). Tag rows & custom-field management stay in form view' },
                ...
                <div className="space-y-[0.3125rem]">
                  {CHANGELOG.map(({ v, ja, en }) => (
                    <div key={v} className="flex gap-2.5 items-start text-[0.6875rem]">
                      ...
                      <span className="text-muted leading-[1.5]">{lang === 'ja' ? ja : en}</span>
                    </div>
                  ))}
                </div>
              </div>
