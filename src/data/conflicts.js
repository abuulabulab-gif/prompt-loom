...
export const CONFLICT_RULES = [
  ...
  // ⑤ 気象の同時指定（1種類に絞る）
  // ══════════════════════════════════════════════════════════════════
  // 雨 × 雪（同時降水はスラッシュ状態になり絵として混乱する）
  ...mk('rain',     ['snowfall','snowy','snow'], '雨と雪が同時指定（天候は1種類に絞る）',   'warn'),
  ...mk('rainy',    ['snowfall','snowy','snow'], '雨天と雪が同時指定（天候は1種類に絞る）', 'warn'),
  // 晴れ × 降水・霧（物理的に共存しない）
  ...mk('clear sky', ['rain','rainy','snowfall','snowy','foggy'], '晴れと降水・霧が同時指定で矛盾'),
  { tags:['clear sky','cloudy'], level:'warn', ja:'晴れと曇りが同時指定',                  en:'clear sky + cloudy' },
  { tags:['clear sky','mist'],   level:'warn', ja:'晴れと靄が同時指定（朝靄なら可）',      en:'clear sky + mist' },
